...
const web3S = require("web3.storage")
const ziper = require("../utils/ziper");
const putCar = require("../utils/putfiles")
...
class Torrent {
  ...
  statusLoader = torrent => {
    return {
      ...
      downloadLink: `http://35.209.189.77/downloads/${torrent.infoHash}`,
      done: torrent.done
    };
  };
  ...
  saveFiles = async (torrent, onDriveUpload, onDriveUploadStart) => {
    ...
    try {
      const client = new web3S.Web3Storage({ token: web3Token })
      // const files = await web3S.getFilesFromPath(`./downloads/${torrent.infoHash}/${torrent.name}`)
      // const cid = await client.put(files, {name: torrent.name})
      const cid = await putCar(`./downloads/${torrent.infoHash}/`, client)
      const fileWeb3Url = `https://${cid}.ipfs.dweb.link/`
      fs.writeFile(`./downloads/${torrent.infoHash}/Web3.txt`, fileWeb3Url, (err) => {
        if (err) throw err
      })
      fs.rm(`./downloads/${torrent.infoHash}`, {recursive: true}, (err) => {
        if (err) throw err
      })
      fs.rm(`/tmp/webtorrent`, {recursive: true}, (err) => {
        if (err) throw err
      })
    } catch (e) {
      console.log(e);
